...
#define DISABLE_GLOBAL_INT           		   len9252_handle.isr_api->pf_disable_global_int()			
#define ENABLE_GLOBAL_INT           		   len9252_handle.isr_api->pf_enable_global_int()			
...
#define    ACK_SYNC1_INT                  	 len9252_handle.south_api->exti_api->pf_exti1_clear();
...
#define ECAT_TIMER_ACK_INT        		 	len9252_handle.south_api->timer_api->pf_clear_it();	//TIM_ClearITPendingBit(TIM2, TIM_IT_Update)	//clear timer interrupt pending bit;	
#define    TimerIsr                    	EC_IT_ISR //	SysTick_Handler//						
#define    ENABLE_ECAT_TIMER_INT        len9252_handle.south_api->timer_api->pf_timer_start() ;//SysTick->CTRL |= SysTick_CTRL_TICKINT_Msk;//NVIC_EnableIRQ(TIM2_IRQn) ;	
#define    DISABLE_ECAT_TIMER_INT       len9252_handle.south_api->timer_api->pf_timer_stop() ;//SysTick->CTRL &= ~SysTick_CTRL_TICKINT_Msk;//NVIC_DisableIRQ(SysTick_IRQn/*TIM2_IRQn*/) ;

#define INIT_ECAT_TIMER            len9252_handle.south_api->timer_api->pf_it_init() ;  // SysTick_Config(SystemCoreClock/1000);  
...
UINT8 HW_Init(void)
{
    UINT16 intMask;
	UINT32 data;
    len9252_handle.pf_sys_init();
#ifdef EC_DEBUG
    EC_DEBUG_OUT("EtherCAT Test!\n");
#endif

    len9252_handle.south_api->gpio_api->pf_rst_reset(); //复位9252
	
	len9252_handle.time_api->pf_delay(100); //延时100ms
	
	len9252_handle.south_api->gpio_api->pf_rst_set();

	len9252_handle.time_api->pf_delay(100);
...
#ifdef EC_DEBUG
    EC_DEBUG_OUT("EtherCAT Test successful!\n");
#endif
    return 0;
}
